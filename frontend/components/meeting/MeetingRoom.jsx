...
// const SPEAKING_DEBOUNCE  = 400;
// const SILENCE_GRACE      = 900;
...
//   // ── FIX: Track wall-clock time when this device started recording ─────────
//   // This is the key to fixing the 0:00 timestamp bug.
//   // Each device's Whisper transcript starts at t=0, but devices start recording
//   // at different wall-clock times. We store this ms epoch value and send it
//   // with every audio-chunk so the backend can compute the true timeline offset.
//   const myRecordingStartTimeRef = useRef(null);
//   // ─────────────────────────────────────────────────────────────────────────
...
//   // ── startMyRecording ──────────────────────────────────────────────────────
//   // FIX: Capture wall-clock recordingStartTime the instant recorder starts.
//   // This value is sent with every audio-chunk so the server can store it
//   // alongside the S3 key and pass it to the worker for timeline normalization.
...
//       // ── FIX: Record the exact ms this device started capturing audio ──────
...
//             audioChunk: buf,
//             timestamp: Date.now(),
//             // ── FIX: Send start time so server stores it per-device ─────────
...
//   // ── stopMyRecording ───────────────────────────────────────────────────────
//   // FIX: Final flush chunk also carries recordingStartTime.
...
//           audioChunk: buf,
//           timestamp: Date.now(),
//           // ── FIX: Include on final flush too ─────────────────────────────
//           recordingStartTime: myRecordingStartTimeRef.current,
//           // ─────────────────────────────────────────────────────────────────
//         });
//       }).catch(() => {});
...
//     myRecordingStartTimeRef.current = null;
...
const SPEAKING_THRESHOLD = 0.08, SPEAKING_DEBOUNCE = 400, SILENCE_GRACE = 900;
function useActiveSpeaker(levelsRef) {
  const [activeSpeakerId, setActiveSpeakerId] = useState(null);
...
export default function MeetingRoom({ meetingId, user }) {
  ...
  // ── Track mic/camera state of ALL remote participants ──────────────────
  // { [userId]: { audio: bool, video: bool } }
  ...
  // Broadcast my mic/camera state to all other participants
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Broadcast initial state (both on)
        ...
        socketRef.current.on('participant-joined', ({ userId, displayName }) => {
          if (!mounted) return;
          setParticipantNames(prev => ({ ...prev, [userId]: { ...prev[userId], fullName: displayName } }));
          // Re-broadcast my state so the new joiner sees correct icons
        ...
        // Receive media state from other participants
  ...
  // Toggle video AND broadcast new state to all peers
  const toggleVideo = useCallback(() => {
  ...
  const remoteProps = (uid) => ({
    isMuted: participantMediaState[uid]?.audio === false,
    isCameraOff: participantMediaState[uid]?.video === false,
  });
...
function LocalTile({ videoRef, name, isHost, isAudioEnabled, isVideoEnabled, isScreenSharing, isHandRaised, isPinned, onPin, onFullscreen, large, thumbnail, isFullscreen, spanFull, stream, isActiveSpeaker, onAudioLevel, audioEnabled, audioLevel }) {
  ...
  return (
    <div className={cn('relative bg-slate-800 rounded-xl overflow-hidden group', large ? 'w-full h-full' : thumbnail ? 'w-40 h-28 shrink-0' : 'w-full h-full', spanFull && 'col-span-2')}>
      ...
      {/* Camera off overlay for your own tile */}
      {!isVideoEnabled && (
...
function RemoteTile({ userId, stream, name, isHandRaised, isPinned, onPin, onFullscreen, large, thumbnail, isFullscreen, spanFull, isActiveSpeaker, onAudioLevel, audioLevel, isMuted, isCameraOff }) {
  ...
  // Show camera-off overlay when: remote explicitly turned off camera, OR no video track yet
  ...
  return (
    <div className={cn('relative bg-slate-800 rounded-xl overflow-hidden group', large ? 'w-full h-full' : thumbnail ? 'w-40 h-28 shrink-0' : 'w-full h-full', spanFull && 'col-span-2')}>
      ...
      {/* Camera off overlay — shown to everyone when remote turns off camera */}
      {showCameraOff && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-800 gap-2">
          <div className="w-14 h-14 rounded-full bg-slate-600 flex items-center justify-center">
            {isCameraOff
              ? <CameraOff className="h-7 w-7 text-slate-400" />
              : <span className="text-xl font-bold text-slate-200">{initials}</span>
            }
          </div>
          {!thumbnail && (
            <span className="text-slate-400 text-xs">
              {isCameraOff ? `${name} · Camera off` : name}
            </span>
          )}
        </div>
      )}
      ...
      <div className="absolute bottom-0 left-0 right-0 px-2 py-1.5 bg-gradient-to-t from-black/70 to-transparent flex items-center gap-1.5">
        <span className="text-white text-xs font-medium truncate">{name}</span>
        {isHandRaised && <span>✋</span>}
        {/* Mic off icon — visible to ALL participants when someone mutes */}
